...
public class MarkerDetectionActivity implements OnTouchListener, CvCameraViewListener2 {
    ...
    public MarkerDetectionActivity(MainActivity activity) {
		...
		gameManager = new GameManager(mMainActivity);
    }
    ...
    private void draw3Dobject(MarkerDetector mDetector){
    	float x = 2*((float)mDetector.getMiddleX()) / mDetector.getFrameWidth() - 1;//TODO remove after check
    	float y = 1 - 2*((float)mDetector.getMiddleY()) / mDetector.getFrameHeight();//TODO remove after check
    	Log.i(Draw3Dobject, "draw3Dobject: x = " + x + " y = " + y);//TODO remove after check
    	
    	String debugText = "draw3Dobject: x = " + x + " y = " + y;//TODO remove after check
    	//gameManager.printTopRight(debugText);
    	
    	mMainActivity.mGLView.mRenderer.setCordinates(x, y);//TODO remove after check
