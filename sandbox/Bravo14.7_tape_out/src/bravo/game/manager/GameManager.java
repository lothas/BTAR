...
public class GameManager {
	...
	public GameManager(MainActivity activity) {
		...
		mTextBravoVersion.setText(String.format("Bravo14.7"));
			...
			if(mMainActivity.tracking.mDetectorArry[i].hasLaser()){
				...
				if ( sizeDistRatio < sizeFactor ){ //TODO: find optimal
					...
					if(distBack < distFront){
						...
						mMainActivity.tracking.myRobot.drawHitCirclel(mMainActivity.tracking.mRgba, rollAngle, 2);
						if(!laserHitFlag){
	...
	public void coinCollect(){
		...
		for(int i = 0; i < mNumOfTowers; i++){
			...
			if(!mMainActivity.tracking.mDetectorArry[i].hasCoin()){
				continue;
			}
	...
	public float getDistInRealWorld(float frontX, float frontY, float backX, float backY, double rollAngle) {
		rollAngle = mMainActivity.tracking.myRobot.findRealAngle(rollAngle, (int)((frontY + backY) / 2));
