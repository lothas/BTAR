...
public class MarkerDetector {
    ...
    protected static final String TAG10 = "remove";
	...
	//-------------------------Fields--------------------------//
	private int ghostCounterReset = 10;
    ...
    public boolean trackObject(List<MatOfPoint> frontContours, List<MatOfPoint> backContoursLeft, List<MatOfPoint> backContoursRight){
        ...
        while (frontCounterIterator.hasNext()) {
            ...
            if ((counterSize > MinContourArea*maxArea) && (counterSize < MaxContourArea*maxArea)) { 
				...
				MatOfPoint contourFordelLeft = null;
				MatOfPoint contourFordelRight = null;
				Iterator<MatOfPoint> backContoursLeftIterator = backContoursLeft.iterator();
				while (backContoursLeftIterator.hasNext()) {	
					Iterator<MatOfPoint> backContoursRightIterator = backContoursRight.iterator();
					centerLeft = trackObjectBackWithItraror1(radius[0], backContoursLeftIterator, counterSize,centerLeft,contourFordelLeft);
					if ((centerLeft.x == -1) && (centerLeft.y == -1)){
						continue;
					}
				
					centerRight = trackObjectBackWithItraror2(radius[0], backContoursRightIterator, counterSize,centerLeft,contourFordelRight);
				...
				if (BackdistanceCheck(radius[0],centerLeft, centerRight) == true) {
					...
					flagFound = true;
					if (iRobot == false){
						deleteConters(frontContours,contour);
						deleteConters(backContoursLeft,contourFordelLeft);
						deleteConters(backContoursRight,contourFordelRight);
					}
					return true;
				}
            }
    ...
    private Point trackObjectBackWithItraror2(float frontRadius, Iterator<MatOfPoint> backContoursLeftIterator, double frontCounterSize, Point otherCenter, MatOfPoint contourFordel){
        ...
        //--------------------------------------------------------------------------------
    	while (backContoursLeftIterator.hasNext()) {
            MatOfPoint contour = backContoursLeftIterator.next();
            double counterSize = Imgproc.contourArea(contour);
            if ((counterSize > MinContourArea*frontCounterSize) && (counterSize < MaxContourArea*frontCounterSize)) {
            	...
            	if (distanceFromFront < allowedSistanceFactor * frontRadius){
            		contourFordel = contour;
            		if (BackdistanceCheck(frontRadius,center,otherCenter)){
            			return center;
            		}
            	}               
            }
    ...
    private Point trackObjectBackWithItraror1(float frontRadius, Iterator<MatOfPoint> backContoursLeftIterator, double frontCounterSize, Point otherCenter, MatOfPoint contourFordel){
	...
	//private void deleteConters(List<MatOfPoint> frontContours, List<MatOfPoint> backContoursLeft, List<MatOfPoint> backContoursRight) {
	//	frontContours.remove()
	//	
	//}

	private void deleteConters(List<MatOfPoint> Contours, MatOfPoint contour) {
		if (Contours.remove(contour)){
			Log.i(TAG10, "removed contor");
		}else{
			Log.i(TAG10, "not removed contor");
		}
	}

	public Point trackObjectBack(float frontRadius, List<MatOfPoint> backContours, double frontCounterSize, Point otherCenter, MatOfPoint contourFordel){
    	//--------------------------------------------------------------------------------
        double MinContourArea = 0.2; //TODO find optimal MinContourArea and MaxContourArea
        double MaxContourArea = 5;
        //--------------------------------------------------------------------------------
    ...
    private void setGoust() {
    	goustCounter = ghostCounterReset;
    	frontGoustX = FrontX;
    	backGoustX  = BackX;
    	frontGoustY = FrontY;
    	backGoustY  = BackY;
	...
	public double findRealAngle(double rollAngle, int middleY2) {
	...
	public int getCoinX(){
		mRadiosFromMiddle = 6; //TODO randomize and set only 1 getAngleFromXAxis
		return (int) (mRadiosFromMiddle*getFront2BackLength()*Math.sin(mCoinAngleFromXAxis + getAngleFromXAxisInRadians())) + MiddleX;
	}
	
	public int getCoinY(){
		mRadiosFromMiddle = 6; //TODO randomize and set only 1
	...
	public void setGhostCounterReset(int num){
		ghostCounterReset = num;
	}
}
