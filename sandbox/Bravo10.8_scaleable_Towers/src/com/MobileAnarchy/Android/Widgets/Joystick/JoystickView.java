...
public class JoystickView extends View {
	...
	protected void onDraw(Canvas canvas) {
		...
		canvas.drawLine(cX, cY, (float)handleX, (float)handleY, stickPaint);
		canvas.drawCircle((float)handleX, (float)handleY, (float)handleRadius, handlePaint);
...
//		Log.d(TAG, String.format("onDraw(%.1f,%.1f)\n\n", handleX, handleY));
	...
	private void constrainCircle() {
		...
		if ( radial > movementRadius ) {
			touchX = (double)((diffX / radial) * movementRadius);
			touchY = (double)((diffY / radial) * movementRadius);
		}
	}
...
//			        	Log.d(TAG, "ACTION_POINTER_DOWN: " + pointerId);
	...
	private boolean processMoveEvent(MotionEvent ev) {
		if ( pointerId != INVALID_POINTER_ID ) {
			...
			double x = ev.getX(pointerIndex);
			touchX = x - cX - offsetX;
			double y = ev.getY(pointerIndex);
			touchY = y - cY - offsetY;
...
//        	Log.d(TAG, String.format("ACTION_MOVE: (%03.0f, %03.0f) => (%03.0f, %03.0f)", x, y, touchX, touchY));
	...
	private void reportOnMoved() {
		...
		if (moveListener != null) {
			...
			if (rx || ry) {
				this.reportX = (float)touchX;
				this.reportY = (float)touchY;
...
//				Log.d(TAG, String.format("moveListener.OnMoved(%d,%d)", (int)userX, (int)userY));
	...
	private void calcUserCoordinates() {
		...
		if ( userCoordinateSystem == COORDINATE_CARTESIAN ) {
			...
			userX = (int)(cartY + cartX / 4);
			userY = (int)(cartY - cartX / 4);
