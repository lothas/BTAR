...
public class MainActivityMisc {
	...
	public void onOptionsItemSelected(MenuItem item) {
		...
		if (id == R.id.options) {
			Intent serverIntent = new Intent(mMainActivity,DebugOpt.class);
			mMainActivity.startActivityForResult(serverIntent,Towers);
		}
		if (id == R.id.about) {
...
/*	public void onSharedPreferenceChanged(SharedPreferences prefs, String key) {
   ...
   public void onActivityResult(int requestCode, int resultCode, Intent data) {
    	...
    	case Towers:
    	      if (resultCode == Activity.RESULT_OK) {
    	    	  int num = 0;
    	          int towers = data.getIntExtra("towers", num); 
    	          int ghost = data.getIntExtra("ghost", num); 
    	          Log.v("number",""+towers);
    	          Log.v("number",""+ghost);
    	          mMainActivity.mGameManager.setNumOfTowers(towers);
    	          mMainActivity.tracking.setNumOfTowers(towers);
    	          mMainActivity.mGLView.mRenderer.setNumOfTowers(towers);
    	          mMainActivity.tracking.setGhostCounterReset(ghost);
