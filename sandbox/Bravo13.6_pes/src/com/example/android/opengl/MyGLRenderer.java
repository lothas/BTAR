...
public class MyGLRenderer implements GLSurfaceView.Renderer {
	...
	private static final String  TAG1              = "MyGLRenderer2";
	//-------------------------Static----------------------------//
    ...
    private AssetObj mLaserWarning;
    ...
    private  MarkerDetector[] detectorArray;
    ...
    public void onSurfaceCreated(GL10 gl, EGLConfig config) {
        ...
        objX = -0.5f; 
        objY = 0.8f;
        setNumOfTowers(mMainActivity.mGameManager.mNumOfTowers);
        detectorArray = new MarkerDetector[num_of_towers];
        for(int i = 0 ; i < num_of_towers ; i++){
        	detectorArray[i] = new MarkerDetector();
        	detectorArray[i].prepareGame(screenWidth, screenHeight, BLUE_COLOR, RED_COLOR, 4);
        }
    ...
    private void load3dObjects(){
        ...
        mLaser = new AssetObj(mMainActivity, "greenLaser.obj","greenLaser.mtl");
        mLaserWarning = new AssetObj(mMainActivity, "lazerWarning.obj","lazerWarning.mtl");
    }
    ...
    public float scaleTrackObjects(MarkerDetector mDetector){
    	...
    	Log.i(TAG1, "MyGLRenderer: mDetector.getFront2BackLengthInRealWorld(rollAngle) = " + mDetector.getFront2BackLengthInRealWorld(rollAngle));
    	return 5*(mDetector.getFront2BackLengthInRealWorld(rollAngle)) / mDetector.getFrameWidth();
    }
    
    public void setTrackObjects(MarkerDetector[] mDetector){
    	for(int i = 0 ; i < num_of_towers ; i++){
    		detectorArray[i] = mDetector[i];
        }
    }
    ...
    final SensorEventListener sensorEventListener = new SensorEventListener() {
		public void onSensorChanged(SensorEvent sensorEvent) {
			if (sensorEvent.sensor.getType() == Sensor.TYPE_ORIENTATION) {
				...
				Log.d(TAG1, "Roll: " + String.valueOf(rollAngle));
			}
		}
    ...
    public void drawObj(GL10 gl, MarkerDetector mDetector){
		...
		float scale = 1f * scaleTrackObjects(mDetector);
    ...
    public void drawCoin(GL10 gl){
    	for(int i = 0 ; i < num_of_towers ; i++){
    		if (detectorArray[i].hasCoin() == true){
    			drawCoinToWorld(gl, detectorArray[i]);
    		}
        }
    }
    ...
    private void drawLaser(GL10 gl) {
    	for(int i = 0 ; i < num_of_towers ; i++){
    		if (detectorArray[i].hasLaser() == true){
    			drawLaserToWorld(gl, detectorArray[i]);
    		}
    		if (detectorArray[i].hasLaserWarning() == true){
    			drawLaserToWarningWorld(gl, detectorArray[i]);
    		}
    ...
    private void drawLaserToWarningWorld(GL10 gl, MarkerDetector mDetector) {
    	if (mDetector.isDetected() == false) {
			return;
		}
		// Save the current matrix.
