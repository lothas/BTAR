...
public class insertColor {
	//-------------------------Fields--------------------------//
	protected static final String TAG1 = "calibration";
		...
		for (i = x - 25; i < x + 25; i++){
			for (j = y - 25; j < y + 25; j++){
				if (imgHSV.get(j,i)[0] > hMax){
					hMax = (imgHSV.get(j,i)[0] + 20 < 255 ? imgHSV.get(j,i)[0] + 20 : 255);
				}
				if (imgHSV.get(j,i)[1] > sMax){
					sMax = (imgHSV.get(j,i)[1] + 40 < 255 ? imgHSV.get(j,i)[1] + 40 : 255);
				}
				if (imgHSV.get(j,i)[2] > vMax){
					vMax = (imgHSV.get(j,i)[2] + 60 < 255 ? imgHSV.get(j,i)[2] + 60 : 255);
				}
				if (imgHSV.get(j,i)[0] < hMin){
					hMin = (imgHSV.get(j,i)[0] - 20 > 0 ? imgHSV.get(j,i)[0] - 20 : 0);
				}
				if (imgHSV.get(j,i)[1] < sMin){
					sMin = (imgHSV.get(j,i)[1] - 40 > 0 ? imgHSV.get(j,i)[1] - 40 : 0);
				}
				if (imgHSV.get(j,i)[2] < vMin){
					vMin = (imgHSV.get(j,i)[2] - 60 > 0 ? imgHSV.get(j,i)[2] - 60 : 0);
				}
				Log.i(TAG1, "deliverTouchEvent: ");
				Log.i(TAG1, "hMax : " + hMax);
				Log.i(TAG1, "sMax : " + sMax);
				Log.i(TAG1, "vMax : " + vMax);
				Log.i(TAG1, "hMin : " + hMin);
				Log.i(TAG1, "sMin : " + sMin);
				Log.i(TAG1, "vMin : " + vMin);
			}
		}
		drawCounter(x, y);
