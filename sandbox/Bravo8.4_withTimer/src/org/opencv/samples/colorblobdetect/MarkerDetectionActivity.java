...
public class MarkerDetectionActivity implements OnTouchListener, CvCameraViewListener2 {
    ...
    public void onCameraViewStarted(int width, int height) {
        ...
        mDetector_BlueRed.prepareGame(width, height, BLUE_COLOR, RED_COLOR, resizeFactor); //TODO 4 is resizefactor
        mDetector_GreenRed.prepareGame(width, height, GREEN_COLOR, RED_COLOR, resizeFactor);
        mDetector_GreenBlue.prepareGame(width, height, GREEN_COLOR, BLUE_COLOR, resizeFactor);
    ...
    public Mat onCameraFrame(CvCameraViewFrame inputFrame) {
		...
		if (colorsAreCalibrated == true) {
			...
			List<MatOfPoint> blueContours = mDetector_BlueRed.getContours(mPyrDownMat, BLUE_COLOR);
			long  startG = System.currentTimeMillis();//start of the func
			List<MatOfPoint> GreenContours = mDetector_BlueRed.getContours(mPyrDownMat, GREEN_COLOR);
			long  startR = System.currentTimeMillis();//start of the func
			List<MatOfPoint> redContours = mDetector_BlueRed.getContours(mPyrDownMat, RED_COLOR);
			...
			mDetector_BlueRed.trackObject(blueContours, redContours);
			mDetector_GreenRed.trackObject(GreenContours, redContours);
			mDetector_GreenBlue.trackObject(GreenContours, blueContours);
			long  detactonEndOld = System.currentTimeMillis();//time before draw3Dobject
			//gameManager.printTopRight(debugText + PyrDownText + "\nOld detaction time: " + (detactonEndOld - detactonStartOld) + "  ");
			...
			draw3Dobject(mDetector_BlueRed,mDetector_GreenRed,mDetector_GreenBlue);
